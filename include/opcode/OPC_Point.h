...
#define _Y 1
#define _Z 2
#define _FORCE_DWORD 0x7fffffff
	...
	class ICEMATHS_API Point
	{
		...
		inline_					Point(float f[3]) : x(f[__X]), y(f[_Y]), z(f[_Z])			{}
		//! Copy constructor
		...
		inline_	Point&			Set(float f[3])							{ x  = f[__X];	y  = f[_Y];	z  = f[_Z];			return *this;	}
		...
		//! Adds a vector
		inline_	Point&			Add(float f[3])							{ x += f[__X];	y += f[_Y];	z += f[_Z];			return *this;	}
		//! Adds vectors
		...
		//! Subtracts a vector
		inline_	Point&			Sub(float f[3])							{ x -= f[__X];	y -= f[_Y];	z -= f[_Z];			return *this;	}
		//! Subtracts vectors
